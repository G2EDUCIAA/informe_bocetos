...
\begin{figure}[!h]
\centering
\includegraphics[width=5 cm]{figuras/primer_proy17.png}\\
...
\fbox{\parbox[b]{\linewidth}{\textbf{\underline{\emph{IMPORTANTE:}}} Si bien la creación de un Path Mapping nos permite emular el comportamiento del
titileo de LEDs, se producen conflictos cuando se depura con la propia placa,pues bloquea el uso de breakpoints.\\Si se cuenta con la placa y se desea hacer Debug con la misma, debe borrar el \textit{Path Mapping} creado. }}
...
\begin{figure}[h]
\centering
\includegraphics[width=10 cm]{figuras/dep1.png}\\
\caption{Configuración para poder hacer Debug sobre la placa: pestaña Main}
\label{dep1}
\end{figure}

Se utiliza OpenOCD (una herramienta OpenSource creada para estos propósitos) para hacer el nexo JTAG-GDB mediante la conexión USB.
Para configurar esta herramienta, desde el menú $Run \longrightarrow Debug Configurations… $ se debe crear un módulo nuevo de \textbf{'Debug configuration'} del tipo \textbf{'GDB OpenOCD Debugging'}.
A continuación,coloque los valores que se muestran en las Figuras \ref{dep1} y \ref{dep2}. Las mismas se listan a continuación:

\begin{enumerate}

\item[$\bullet$]Pestaña Main:

\begin{enumerate}
\item[1] Name:\textbf{Blinking OpenOCD}
\item[2] Project:\textbf{blinking}
\item[3] C/C++ Application:\textbf{C:/CIAA/Firmware/out/bin/blinking.axf}
\item[4] Tildar \textbf{Disable auto build}
\item[5] Build configuration: Default
\end{enumerate}

\item[$\bullet$]Pestaña Debugger:

\begin{enumerate}

\item[1] OpenOCD Setup: Destildar la opción \textit{Start OpenOCD locally}
\item[2]
\begin{enumerate}

\item[a]Executable: \textbf{C:/CIAA/cygwin/usr/arm-none-eabi/bin/arm-none-eabigdb.exe}
\item[b]\textit{Other options} y \textit{Commands}: dejarlo como está
\item[c]Destildar \textit{‘Force thread list update on suspend’}
\end{enumerate}
...
\begin{figure}[!h]
\centering
\includegraphics[width=10 cm]{figuras/dep2.png}\\
\caption{Configuración para poder hacer Debug sobre la placa: pestaña Debugger}
\label{dep2}
\end{figure}
\end{center}

Para hacer Debug, es necesario que se abra un puerto a través del OpenOCD. Hay muchas formasde hacerlo. En este tutorial, lo haremos usando la consola CygWin, que viene incluida con el instalador del CIAA-IDE.\\
Al abrir la consola. Aparece una ventana como la que se muestra en la Figura \ref{dep3}

\begin{center}
\begin{figure}[!h]
\centering
\includegraphics[width=5 cm]{figuras/dep3.png}\\
\caption{Consola CygWin}
\label{dep3}
\end{figure}
\end{center}
...
cd C:/CIAA/Firmware & Nos posiciona en la carpeta Firmware (si usamos los Path por defecto)\\ \hline
make openocd & Comienza a correr el servicio del OpenOCD\\ \hline
%obj & Todos los archivos fuentes son compilados a object files y almacenados en este directorio\\ \hline
\end{tabular}
\caption{Tabla 2}
\label{Tab2}
\end{center}
\end{table}
%tecnicamente se describe que pasa con la placa virgen pero este tema se ha tratado debido a que la explicacion se baso en el documento PRIMEROS PASOS CON EL HARDWARE
...
\subsubsection{Posible Problema: No reconocimiento}

Si se desconecta la placa de la PC y se la vuelve a conectar en un puerto USB distinto al anterior, Windows detectará el nuevo Hardware
encontrado, instalará los drivers y la placa se encenderá; pero al intentar hacer Debug, en la consola CygWin es posible que emerja un mensaje de error como la de la Figura \ref{dep4}.

\begin{center}
\begin{figure}[!h]
\centering
\includegraphics[width=8 cm]{figuras/dep4.png}\\
\caption{Consola CygWin}
\label{dep4}
\end{figure}
\end{center}

Si se hiciera un par de pruebas, este error también corresponde al que se daría si la placa no estuviera conectada. La causa es que el driver que se acaba de instalar no es el corregido por elprograma Zadig, sino es el instalado por el instalador del Software-IDE. Para solucionar este inconveniente simplemente hay que repetir los pasos que se destacan sobre el tutorial,  y con ello, la placa vuelve a funcionar correctamente.
