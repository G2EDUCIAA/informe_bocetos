...
instalan las herramientas para Debug. Se utiliza OpenOCD (una herramienta OpenSource creada para estos propósitos) para hacer el nexo JTAG-GDB mediante la conexión USB. Para configurar esta herramienta, desde el menú $Run \longrightarrow Debug Configurations… $ se debe crear un módulo nuevo de \textbf{'Debug configuration'} del tipo \textbf{'GDB OpenOCD Debugging'}. A continuación,coloque los valores que se muestran en las Figura \ref{dep1} y \ref{dep2}. Las mismas se listan a
continuación:
...
\begin{figure}[!h]
\centering
\includegraphics[width=8 cm]{figuras/dep1.png}\\
