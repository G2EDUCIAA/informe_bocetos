...
La Figura  ilustra las declaraciónes realizadas:
\begin{figure}[H]
\centering
...
\item[•]\textbf{Paso 12}: Sobre el archivo fuente del proyecto debe especificarse dentro de la función principal \textit{main ()}, el inicio del Sistema Operativo. Esto se realiza a través de la instrucción \textit{StartOs(A)}, sobre el parámetro A, el usuario debe ingresar el modo de aplicación definido en el archivo de configuración (en este caso \textit{AppMode1}).

\item[•] \textbf{Paso 13}: Para declarar cada Tarea sobre el archivo fuente, se debe ingresar una sintaxis similar a la que se realiza sobre el archivo de configuración. La Figura \ref{tareaenarchfuente} ilustra la declaración de la Tarea INICIALIZAR sobre el archivo de configuración.
\begin{figure}[H]
\centering
\includegraphics[width=5 cm]{figuras/f49.png}
\caption{Ejemplo de definición de Tarea sobre archivo fuente.}
\label{tareaenarchfuente}
\end{figure}
