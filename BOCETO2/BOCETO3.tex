...
\end{figure}
\end{center} 
\subsubsection{Implementacion de ejemplo de salidas digitales}
...
\end{figure}
\end{center}
\subsubsection{Implementacion de ejemplo de entradas y salidas digitales}
...
Estado \textit{\textbf{INICIAL}}: Estado inicial en el cual, si se oprime la tecla \textit{TEC1}, se ejecuta la acción \textit{LED$\_$R}.

Estado \textit{\textbf{ROJO}}: El codigo establece que al oprimir la tecla \textit{TEC1}, se ejecuta la acción \textit{P$\_$LED1}. 

Estado \textit{\textbf{LED$\_$1}}: El codigo establece que al oprimir la tecla \textit{TEC1}, se ejecuta la acción \textit{P$\_$LED2}.

Estado \textit{\textbf{LED$\_$2}}: El codigo establece que al oprimir la tecla \textit{TEC1}, se ejecuta la acción \textit{P$\_$LED3}.

Estado \textit{\textbf{LED$\_$3}}: El codigo establece que al oprimir la tecla \textit{TEC1}, se ejecuta la acción \textit{P$\_$LED3}.  

\underline{Eventos}:

Evento \textit{\textbf{PRESIONO}}: Identifica si se oprime la tecla \textit{TEC1}.

Evento \textit{\textbf{PRESIONO$\_$4}}: Identifica si se oprime la tecla \textit{TEC1} en la primer secuencia.

Evento \textit{\textbf{PRESIONO$\_$7}}: Identifica si se oprime la tecla \textit{TEC1} en la segunda secuencia.

Evento \textit{\textbf{PRESIONO$\_$10}}: Identifica si se oprime la tecla \textit{TEC1} en la tercer y ultima secuencia.

\underline{Acciones}:

Accion \textit{\textbf{LED$\_$R}}: Encendido de \textit{LEDR},\textit{LEDB},\textit{LEDG} según corresponda.

Accion \textit{\textbf{P$\_$LED$\_$1}}: Encendido de \textit{LED1}.

Accion \textit{\textbf{P$\_$LED$\_$2}}: Encendido de \textit{LED2}.

Accion \textit{\textbf{P$\_$LED$\_$3}}: Encendido de \textit{LED3}.

La \ref{Fig22} especifica el diagrama de estados del ejemplo:
\begin{center}
\begin{figure}[!h]
\centering
\includegraphics[width=7 cm]{figuras/f4.png}\\
\caption{Diagrama de estados de ejemplo de entradas y salidas digitales}
\label{Fig22}
\end{figure}
\end{center}
\subsubsection{Implementacion de segundo ejemplo de entradas y salidas digitales}
...
Estado \textit{\textbf{INICIAL}}: Estado inicial en el cual, en el caso de la ocurrencia del evento \textit{NO$\_$APRETO}, el programa se establece en el mismo estado;mientras que para el caso en el caso de la ocurrencia del evento \textit{APRETO1}, el programa se establece en el estado \textit{ESTADO1}; en el caso de la ocurrencia del evento \textit{APRETO2}, el programa se establece en el estado \textit{ESTADO2}. 

Estado \textit{\textbf{ESTADO1}}: Sobre dicho estado se realiza la secuencia de luces y la muestra de los digitos decimales sobre el display.

Estado \textit{\textbf{ESTADO2}}: Sobre dicho estado se muestra de un digito decimal en caso de que se oprima la tecla \textit{TEC2}.

\underline{Eventos}:

Evento \textit{\textbf{NO$\_$APRETO}}: Identifica si no se ha oprimido la tecla \textit{TEC1} o la tecla \textit{TEC2}.

Evento \textit{\textbf{APRETO1}}: Identifica si se oprime la tecla \textit{TEC1}.

Evento \textit{\textbf{APRETO2}}: Identifica si se oprime la tecla \textit{TEC2}.
...
\caption{Diagrama de estados para segundo ejemplo de entradas y salidas digitales.}
\label{Fig27}
\end{figure}
\end{center}

\subsubsection{Implementacion de ejemplo de entradas analógicas y uso de display 7 segmentos}
...
Estado \textit{\textbf{UN$\_$VOLT}}: Estado en el cual, la tensión leída es igual a 1[V].

Estado \textit{\textbf{DOS$\_$VOLT}}: Estado en el cual, la tensión leída es igual a 2[V].

Estado \textit{\textbf{TRES$\_$VOLT}}: Estado en el cual, la tensión leída es igual a 3[V].

Estado \textit{\textbf{CUATRO$\_$VOLT}}: Estado en el cual, la tensión leída es igual a 4[V].

Estado \textit{\textbf{CINCO$\_$VOLT}}: Estado en el cual, la tensión leída es igual a 5[V].

Estado \textit{\textbf{INTERMEDIO}}: Estado inicial en el cual, la tensión leída no es igual a 1[V],2[V],3[V],4[V],o 5[V].
\begin{center}
\begin{figure}[!h]
\centering
\includegraphics[width=6 cm]{figuras/f7.png}\\
\caption{Diagrama de estados para ejemplo de entradas analógicas.}
\label{Fig25}
\end{figure}
\end{center}
\underline{Eventos}:

Evento \textit{\textbf{LEYO$\_$1}}: Identifica si la lectura realizada sobre el ADC corresponde a 1[V].

Evento \textit{\textbf{LEYO$\_$2}}: Identifica si la lectura realizada sobre el ADC corresponde a 2[V].

Evento \textit{\textbf{LEYO$\_$3}}: Identifica si la lectura realizada sobre el ADC corresponde a 3[V].

Evento \textit{\textbf{LEYO$\_$4}}: Identifica si la lectura realizada sobre el ADC corresponde a 4[V].

Evento \textit{\textbf{LEYO$\_$5}}: Identifica si la lectura realizada sobre el ADC corresponde a 5[V].

Evento \textit{\textbf{NOES1}}: Identifica si la lectura realizada sobre el ADC es distinto a 1[V].

Evento \textit{\textbf{NOES2}}: Identifica si la lectura realizada sobre el ADC es distinto a 2[V].

Evento \textit{\textbf{NOES3}}: Identifica si la lectura realizada sobre el ADC es distinto a 3[V].

Evento \textit{\textbf{NOES4}}: Identifica si la lectura realizada sobre el ADC es distinto a 4[V].

Evento \textit{\textbf{NOES5}}: Identifica si la lectura realizada sobre el ADC es distinto a 5[V].

Evento \textit{\textbf{NOESNINGUNO}}: Identifica si la lectura realizada sobre el ADC no corresponde a 1[V],2[V],3[V],4[V] y 5[V].

\subsubsection{Implementación de ejemplo de salidas analógicas}
...
Estado \textit{\textbf{LEO$\_$POTENCIOMETRO}}: Estado inicial, en el cual se determina la lectura analógica.

Estado \textit{\textbf{SALIDA$\_$2}}: Estado en el cual se realiza una salida analógica igual a 2[V], en el caso que la lectura realizada sea superior a 2[V].

Estado \textit{\textbf{SALIDA$\_$3}}: Estado en el cual se realiza una salida analógica igual a 3[V], en el caso que la lectura realizada sea superior a 4[V].

\underline{Eventos}:

Evento \textit{\textbf{MAYOR$\_$2}}: Identifica si la lectura realizada sobre el ADC es superior a 2[V].

Evento \textit{\textbf{MAYOR$\_$4}}: Identifica si la lectura realizada sobre el ADC es superior a 4[V].

Evento \textit{\textbf{MENOR$\_$4}}: Identifica si la lectura realizada sobre el ADC es inferior a 4[V].

Evento \textit{\textbf{NINGUNO}}: Identifica si la lectura realizada sobre el ADC es inferior a 2[V].
...
\caption{Diagrama de estados de ejemplo de salidas analógicas.}
\label{Fig28}
\end{figure}
\end{center}
